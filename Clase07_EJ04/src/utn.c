...
int utn_getWords(char* pArray, int longitud)
{
	...
	strncpy (pArray, buffer, strlen(buffer));
...
/**
 * \biref Verifica si la cadena ingresada en entera
...
/**
 * \biref Verifica si la cadena ingresada en flotante
 * \param pArray Puntero al espacio de memoria donde se dejara el resultado de la funcion
 * \return Retorna 0 (EXITO) si se obtiene un numero y -1 (ERROR) si no
 */
static int getFloat(float* pArray)
...
/**
 * \biref Verifica si la cadena ingresada es entera
 * \param pArray pArray de caracteres a ser analizados
 * \return Retorna '1'(VERDADERO) si la cadena es entera | '0'(FALSO) si no lo es
 */
static int esNumericaInt(char* pArray)
{
	int retorno = 1;
...
/**
 * \biref Verifica si la cadena ingresada es flotante
 * \param pArray pArray de caracteres a ser analizados
 * \return Retorna '1'(VERDADERO) si la cadena es flotente | '0'(FALSO) si no lo es
 */
static int esNumericaFloat(char* pArray)
{
	int retorno = 1;
 ...
 * \biref
 * \param
 * \return (1) Encontro un Mayor  (0) Encontro dos numeros iguales  (-1)  Error
 */
int utn_calcularMayorInt(int* pResultado, int num1, int num2)
{
	...
	else if(pResultado != NULL && num1 == num2)
	{
		retorno = 0;
	}

	printf("numero mayor presultado: %d\n" ,*pResultado);
...
int utn_ingresarArrayInt(int pArray[], int len, int min, int max)
{
	...
	if(pArray != NULL && pArray >0)
	{
		...
		{
			printf("Indice: %d -- ", i);
			if(utn_getNumeroInt(&pArray[i], "Ingrese Valor: ", "ERROR ", min, max, 2) == -1)
			{
				printf("\nERROR al cargar\n");
				retorno = -1;
				break;
		...
		if(retorno == 0)
		{
			printf("\nFinalizo la carga exitosamente\n");
		}
	}
	return retorno;
}
...
/**
 * \biref	Inicializa un array insertantodo en cada posicion del array un caracter
 * \param	insertarCharacter Recibe el character a insertar
 * \param	pArray  Recibe el array
 * \param	len Longitud del array
 * \return	-1 (ERROR) -- 0 (OK)
 */
...
/**
 * \biref 	Cuenta la cantidad de espacios que hay dentro de un array de characters
 * \param	pArray  Recibe el array
 * \param	len Longitud del array
 * \param	cant Devuelve la cantidad de espacios que encotro en el array
 * \return	-1 (ERROR) -- 0 (OK)
 */
int utn_contarEspaciosArrayChar(int* cant, char pArray[], int len)
{
	...
	if(pArray != NULL && len > 0 && cant != NULL)
	{
		for(int i=0; i<len && pArray[i] != '\0'; i++)
		{
			if(pArray[i] == ' ')
...
/**
 * \biref 	Recorre el array sustituyendo los espacios por el caracter pasado como parametro
 * \param	pArray  Recibe el array
 * \param	len Longitud del array
 * \param	insertarCharacter Recibe el caracter a introducir en donde encuetre un espacio en la cadena
 * \return	-1 (ERROR) -- 0 (OK)
 */
